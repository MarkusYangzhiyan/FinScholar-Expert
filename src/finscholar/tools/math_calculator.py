...
class MathCalculator:
    ...
    def _validate_ast_size(self,ast_parsed_expression:ast.AST) -> None:
        ...
        if node_count > self._max_ast_nodes:
            raise UnsafeExpressionError(
                "表达式过于复杂："
                f"AST 节点数量 {node_count} 超过限制 {self._max_ast_nodes} "
    ...
    def _evaluate(
        ...
        raise UnsafeExpressionError(
            f"不允许的表达式语法：{type(ast_parsed_expression).__name__}"
        )
    ...
    def _evaluate_unary(
        self,
        ast_parsed_expression : ast.UnaryOp,            variables : dict[str,Decimal]
    ) -> Decimal:
        """处理正号和负号。"""
        ...
        raise UnsafeExpressionError(
            f"不允许的一元运算：{type(ast_parsed_expression.op).__name__}"
        )

    def _evaluate_binary(
        self,
        ast_parsed_expression: ast.BinOp,            variables:dict[str,Decimal]
    ) -> Decimal:
        """处理加、减、乘、除、幂运算。"""
        ...
        raise UnsafeExpressionError(
            f"不允许的二元运算：{type(ast_parsed_expression.op).__name__}"
        )
    ...
    def _evaluate_power(
        ...
        if abs(right) > self._max_power_abs:
            raise UnsafeExpressionError(
                f"指数绝对值不能超过 {self._max_power_abs}"
            )
